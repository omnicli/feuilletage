...
[![docs.rs](https://img.shields.io/docsrs/compote)](https://docs.rs/compote)
[![Crates.io](https://img.shields.io/crates/v/compote.svg)](https://crates.io/crates/compote)
...
- **Powerful derive**: `#[derive(compote::Config)]` with 30+ attributes for
...
Full reference docs: [docs.rs/compote](https://docs.rs/compote)

## Install

```toml
[dependencies]
compote = { version = "0.1", features = ["yaml", "json"] }
```
...
struct ServerConfig {
    ...
    #[compote(range(1, 65535), default = "8080")]
    port: u16,
...
| `regex` | no | `#[compote(regex = "...")]` validation |
| `chrono` | no | `#[compote(datetime = "...")]` date/time validation |

## Layered configuration

Compote was built around the system/user/local triad. Mutability rules let you
...
use compote::{loader, Format, Level};
...
struct AppConfig {
    ...
    #[compote(env = "PORT", range(1, 65535), default = "8080")]
    port: u16,

    #[compote(length(8, 64))]
    password: String,

    #[compote(validate = "is_even")]
    workers: i32,
}
...
```rust
#[derive(compote::Config)]
#[compote(scalar_as = "version")]    // String input becomes {version: "..."}
struct PythonConfig {
    version: Option<String>,
}

#[derive(compote::Config)]
#[compote(array_as = "packages")]    // Array input becomes {packages: [...]}
struct HomebrewConfig {
    #[compote(allow_single)]         // Also accepts a single value as array
    packages: Vec<String>,
}
```
...
```rust
# #[derive(compote::Config)] struct HomebrewConfig { #[compote(default)] pkgs: Vec<String> }
# #[derive(compote::Config)] struct PythonConfig { #[compote(default)] version: Option<String> }
# #[derive(compote::Config)]
# #[compote(scalar_as = "version")]
# struct MiseConfig {
#     #[compote(default = "")] name: String,
#     version: Option<String>,
# }
#[derive(compote::Config)]
...
enum Tool {
    ...
    Python(PythonConfig),
    #[compote(fallback, from_tag = "name")] // Unknown keys land here
    Mise(MiseConfig),
}
...
```rust
use compote::{Config, ContextValue};
...
```bash
cargo run -p compote --example simple_validation --features "json yaml"
```
...
independent `compote` / `compote-macros` versioning.
