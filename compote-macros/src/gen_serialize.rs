...
/// Uses the compote::IsEmpty trait for emptiness checks.
pub(crate) fn get_skip_check_recursive_for_type(ty: &Type) -> proc_macro2::TokenStream {
    ...
    match type_str.as_str() {
            ...
            if let Some(inner_ty) = get_inner_type(ty) {
                ...
                if inner_type_str == "Option" {
                    ...
                    quote! {
                        match value {
                            None => true,
                            Some(inner) => ::compote::IsEmpty::is_empty(inner),
                        }
                    }
                }
            } else {
        ...
        _ => quote! { ::compote::IsEmpty::is_empty(value) },
    }
}
