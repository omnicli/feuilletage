...
//!
//! Generates the runtime validation checks for `#[compote(range(..))]`,
//! `#[compote(regex = ..)]`, `#[compote(length(..))]`,
//! `#[compote(validate = ..)]`, `#[compote(absolute_path)]`, etc.
...
pub(crate) fn generate_validation_code(
    ...
    if let Some(ref pattern) = attrs.regex {
        ...
        validations.push(quote! {
            {
                let re = compote::regex::Regex::new(#pattern).map_err(|e| {
                    compote::Error::InvalidValue {
                        path: tracker.current_path(),
        ...
        validations.push(quote! {
            {
                ...
                if compote::chrono::NaiveDateTime::parse_from_str(&value_str, #format_str).is_err()
                    && compote::chrono::NaiveDate::parse_from_str(&value_str, #format_str).is_err()
                    && compote::chrono::NaiveTime::parse_from_str(&value_str, #format_str).is_err()
                {
                    let error = compote::Error::InvalidValue {
                        path: tracker.current_path(),
                        message: #error_msg,
