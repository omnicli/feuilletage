...
use crate::attrs::{
    ...
    validate_compote_attributes, ContainerAttributes, DefaultValue, DurationConfig,
...
///
/// All attributes use the `#[compote(...)]` namespace. Unknown container,
/// field, and variant attributes are rejected at compile time. The main
/// `compote` crate owns executable rejection tests because generated code
/// depends on its runtime types.
///
/// # Runnable Examples
///
/// For runnable doctest examples of all attributes, see the main
/// [`compote`](https://docs.rs/compote) library documentation, specifically
...
///     port: i32,
///
///     #[compote(default)]  // Uses Default::default()
///     debug: bool,
///
...
/// ## Default Values
///
/// - `#[compote(default = "value")]` - Use explicit default value (makes field optional)
/// - `#[compote(default)]` - Use `Default::default()` (makes field optional)
/// - `#[compote(default_fn = "function_name")]` - Call function to get default (makes field optional)
///
/// ```text
/// #[derive(Config)]
/// struct Config {
...
///
/// - `#[compote(allow_single)]` - Accept single value as array (for Vec fields)
/// - `#[compote(allow_single = "field")]` - Wrap scalar as object (for struct fields)
/// - `#[compote(allow_map = "key")]` - Accept map notation for Vec
/// - `#[compote(allow_map(key = "field", scalar_as = "value_field"))]` - Full syntax
/// - `#[compote(allow_map(key = "field", order_by = "sort_field"))]` - Sort after conversion
/// - `#[compote(allow_list)]` - Accept array input for HashMap/BTreeMap fields
///
/// ```text
/// #[derive(Config)]
/// struct Config {
...
/// ## Transforms
///
/// - `#[compote(transform = "function_name")]` - Apply transform function
/// - `#[compote(transform_each = "function_name")]` - Transform each item in Vec
/// - `#[compote(relative_path)]` - Shorthand for `transform = "relative_path"`
/// - `#[compote(normalize_path)]` - Shorthand for `transform = "normalize_path"` (resolves `.` and `..`)
/// - `#[compote(duration)]` - Parse duration strings to seconds (default)
/// - `#[compote(duration(ms))]` - Parse duration strings to specified unit (shorthand)
/// - `#[compote(duration(unit = ms))]` - Parse duration strings to specified unit (explicit)
///
...
/// |--------|-------------|-------------|
/// | `#[compote(duration)]` | seconds | Default - converts to seconds |
/// | `#[compote(duration(ms))]` | milliseconds | Shorthand - unit identifier only |
/// | `#[compote(duration(unit = ns))]` | nanoseconds | Explicit - named parameter |
///
/// ### Duration Units
///
/// | Unit | Name | Example |
...
/// ## Field Naming
///
/// - `#[compote(rename = "key")]` - Use different key for serialization/deserialization
/// - `#[compote(aliases = ["alt1", "alt2"])]` - Accept alternative key names
///
/// ```text
/// #[derive(Config)]
/// struct Config {
...
/// ## Validation
///
/// - `#[compote(range(min, max))]` - Numeric range validation
/// - `#[compote(regex = "pattern")]` - String pattern validation (requires `regex` feature)
/// - `#[compote(length(min, max))]` - String/list length validation
/// - `#[compote(validate = "function_name")]` - Custom validation function
/// - `#[compote(datetime = "fmt")]` - Date/time format validation (requires `chrono` feature)
/// - `#[compote(absolute_path)]` - Validate path is absolute
///
/// ```text
/// #[derive(Config)]
/// struct Config {
...
///     percentage: i32,
///
///     #[compote(regex = r"^[a-z0-9_]+$")]
///     username: String,
///
///     #[compote(length(8, 128))]
///     password: String,
///
///     #[compote(validate = "validate_port")]
///     port: i32,
/// }
///
...
///
/// - `#[compote(env = "VAR_NAME")]` - Load from environment variable as fallback
///
/// ```text
/// #[derive(Config)]
/// struct Config {
///     #[compote(env = "DATABASE_URL", default = "localhost")]
...
/// ## Metadata
///
/// - `#[compote(deprecated = "message")]` - Print deprecation warning
/// - `#[compote(secret)]` - Hide value in error messages
/// - `#[compote(mutable_by = ["level1", "level2"])]` - Restrict which levels can set this field
/// - `#[compote(nested)]` - Compose a nested Config type's mutability rules into the parent
///
/// ## Type Coercion
///
/// - `#[compote(coerce)]` - Enable liberal type coercion (string<->int<->float<->bool)
///
/// ## Struct Flattening
///
/// - `#[compote(flatten)]` - Flatten nested struct fields into parent
///
/// ## Serialization Control
///
/// - `#[compote(skip)]` - Always skip serializing this field
/// - `#[compote(skip_if_empty)]` - Skip if field is empty
/// - `#[compote(skip_if_empty_recursive)]` - Skip if field is empty recursively
/// - `#[compote(skip_if_default)]` - Skip if field equals default value
/// - `#[compote(skip_if = "function_name")]` - Custom skip function
/// - `#[compote(serialize_single_as_value)]` - Serialize single-item Vec as value
///
/// # Struct-Level Attributes
///
/// - `#[compote(scalar_as = "field")]` - Wrap scalar input as `{field: value}`
/// - `#[compote(array_as = "field")]` - Wrap array input as `{field: value}`
/// - `#[compote(transform = "fn_name")]` - Run a normalizer function on the raw
...
/// ## Tagged Enums
///
/// Use `#[compote(tag = "field")]` for internally tagged enums:
///
/// ```text
/// #[derive(Config)]
/// #[compote(tag = "type")]
...
/// ## Untagged Enums
///
/// Use `#[compote(untagged)]` for untagged enums (tries each variant in order):
///
/// ```text
/// #[derive(Config)]
/// #[compote(untagged)]
...
/// ## Enum Attributes
///
/// - `#[compote(rename = "tag")]` - Variant tag name
/// - `#[compote(alias = "alt")]` - Alternative tag value
/// - `#[compote(rename_all = "case")]` - Case conversion for all variants
///   (snake_case, camelCase, PascalCase, kebab-case, SCREAMING_SNAKE_CASE)
#[proc_macro_derive(Config, attributes(compote))]
pub fn derive_config(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);

    if let Err(error) = validate_compote_attributes(&input) {
    ...
    // Create extended generics with __CompoteS and __CompoteL for FromContextValue impl.
    ...
    // 3. So for MyConfig<T>, we generate: impl<T, __CompoteS, __CompoteL> FromContextValue<__CompoteS, __CompoteL> for MyConfig<T>
    let mut extended_generics = generics.clone();
    extended_generics
        .params
        .push(syn::parse_quote!(__CompoteS: compote::CustomSource));
    extended_generics
        .params
        .push(syn::parse_quote!(__CompoteL: compote::CustomLevel));
    ...
    quote! {
        const _: () = {
            use ::compote as compote;
            #[allow(unused_imports)]
            use ::compote::__private::{
                format, vec, Cow as __CompoteCow, HashSet as __CompoteHashSet,
                IndexMap as __CompoteIndexMap, ToString as __CompoteToString, serde, serde_json,
            };
...
fn generate_struct_impl(
    ...
    let fields = match fields {
        ...
        Fields::Unnamed(_) => panic!(
            "Config can only be derived for tuple structs when `#[compote(transparent)]` is set and there is exactly one field"
        ),
    ...
    let remaining_obj_creation = if has_flattened_fields {
        ...
        quote! {
            ...
            let __excluded_keys: __CompoteHashSet<&str> = [#(#excluded_keys),*].into_iter().collect();
            let __remaining_obj: __CompoteIndexMap<String, compote::ContextValue<__CompoteS, __CompoteL>> = obj.iter()
                .filter(|(k, _)| !__excluded_keys.contains(k.as_str()))
                .map(|(k, v)| (k.clone(), v.clone()))
                .collect();
            let __remaining_value = compote::ContextValue::object(
                __remaining_obj,
                value.context().clone(),
            );
        }
    ...
    let container_transform_call = if let Some(ref fn_name) = container_attrs.transform {
        ...
        quote! {
            ...
            let __compote_transformed_value = {
                let mut __compote_v = value.clone();
                let __compote_ctx = __compote_v.context().clone();
                #path(&mut __compote_v, &__compote_ctx)?;
                __compote_v
            };
            let value = &__compote_transformed_value;
        }
    } else {
        quote! {}
    };
...
fn generate_transparent_struct_impl(
    ...
    let local_var = field
        .ident
        .clone()
        .unwrap_or_else(|| syn::Ident::new("__compote_inner", proc_macro2::Span::call_site()));
    ...
    let expanded = quote! {
        ...
        impl #impl_generics compote::MutabilityInfo for #name #ty_generics #where_clause {
            fn mutability_constraints() -> compote::MutabilityConstraints {
                compote::MutabilityHashMap::default()
            }
        }
    };
    ...
    if is_vec_allow_single || has_allow_map {
        ...
        if let Some(ref transform_fn) = attrs.transform {
            ...
            quote! {
                ...
                let #field_name = compote::FromContextValue::from_context_value(&__transformed, tracker)?;
            }
        } else {
            quote! {
                let #field_name = compote::FromContextValue::from_context_value(value, tracker)?;
            }
        }
    }
}
    ...
    let normalization_code = if let Some(ref allow_map_config) = attrs.allow_map {
        ...
        let map_to_array_body = if let Some(ref scalar_as_field) = allow_map_config.scalar_as_field
        {
            quote! {
                ...
                compote::ContextValue::array(items, value.context().clone())
            }
        } else {
            quote! {
                was_from_map = true;
        ...
        let map_to_array = quote! {
            compote::ContextValue::Object(map, _) => {
                ...
                if is_single_item {
                    ...
                    compote::ContextValue::array(vec![value.clone()], value.context().clone())
                } else {
        ...
        quote! {
            ...
            let array_value = match value {
                compote::ContextValue::Array(_, _) => value.clone(),
                #map_to_array
                #single_value_arm
            };
        }
        ...
        quote! {
            ...
            let array_value = match value {
                ...
                compote::ContextValue::Object(map, _) => {
                    ...
                    let key_fields = <#elem_type as compote::AllowMapKeys>::map_key_fields();
                    let is_single_item = key_fields.iter().any(|k| map.contains_key(*k));
        ...
        quote! {
            ...
            let array_value = match value {
                compote::ContextValue::Array(_, _) => value.clone(),
                _ => compote::ContextValue::array(vec![value.clone()], value.context().clone()),
            };
        }
    } else {
        quote! {
        ...
        quote! {
            let mut #raw_var_name: Option<#field_type> = {
                ...
                let result = match #lookup_chain {
                    Some(field_value) => {
                        match compote::FromContextValue::from_context_value(field_value, tracker) {
                            Ok(v) => Some(Some(v)),
                            Err(_) => Some(None),
                        }
                    }
...
fn generate_mutability_info_impl(
    ...
    let nested_insertions = nested_fields.iter().map(|(field_type, prefixes)| {
        let insertions = prefixes.iter().map(|prefix| {
            if prefix.is_empty() {
                quote! {
                    map.extend(<#field_type as compote::MutabilityInfo>::mutability_constraints());
                }
            } else {
                quote! {
                    for (mut path, allowed_levels) in
                        <#field_type as compote::MutabilityInfo>::mutability_constraints()
                    {
                        path.insert(0, '.');
    ...
    // Use compote's re-exported HashMap which works in both std and no_std
    quote! {
        impl #impl_generics compote::MutabilityInfo for #name #ty_generics #where_clause {
            fn mutability_constraints() -> compote::MutabilityConstraints {
                #(#static_arrays)*
                let mut map = compote::MutabilityHashMap::new();
                #(#map_insertions)*
                #(#nested_insertions)*
                map
            }
...
/// Generate AllowMapKeys implementation for a struct deriving `compote::Config`.
///
...
///
/// - If the struct has `#[compote(allow_map(key = field))]`, returns that field's names
...
fn generate_allow_map_keys_impl(
    ...
    quote! {
        impl #impl_generics compote::AllowMapKeys for #name #ty_generics #where_clause {
...
/// Generate template resolution code for fields with #[compote(template)] attribute.
///
        ...
        .map(|(field_name, field_name_str, _field_type, attrs)| {
            ...
            quote! {
                ...
                let #field_name = {
                    ...
                    match compote::interpolate_template(template_str, &__template_field_values, #vec_delimiter) {
                        Ok(interpolated) => {
...
fn generate_value_wrapping(
    ...
    let scalar_wrap = if has_scalar_as {
        ...
        quote! {
            compote::ContextValue::String(_, _) | compote::ContextValue::Int(_, _) | compote::ContextValue::Float(_, _) | compote::ContextValue::Bool(_, _) => {
                // Scalar input - wrap as {scalar_as_field: value}
                let mut wrapped_obj = __CompoteIndexMap::default();
                wrapped_obj.insert(#scalar_field.to_string(), value.clone());
                __CompoteCow::Owned(compote::ContextValue::object(wrapped_obj, value.context().clone()))
            }
        }
    } else {
        quote! {}
    ...
    let object_wrap = if has_struct_allow_map {
                ...
                match single_value {
                    compote::ContextValue::String(_, _) | compote::ContextValue::Int(_, _) | compote::ContextValue::Float(_, _) | compote::ContextValue::Bool(_, _) => {
                        ...
                        wrapped_obj.insert(#key_field.to_string(),
                            compote::ContextValue::string(single_key.clone(), single_value.context().clone()));
        ...
        quote! {
            compote::ContextValue::Object(map, _) if map.len() == 1 => {
    ...
    if has_any_wrap {
        quote! {
            ...
            let value: __CompoteCow<'_, compote::ContextValue<__CompoteS, __CompoteL>> = match value {
                #scalar_wrap
                #array_wrap
                #object_wrap
                _ => __CompoteCow::Borrowed(value),
...
fn generate_skip_field_default(
    ...
    // 3. Use Default::default() (from #[compote(default)] or implicit)
...
fn generate_flatten_deserialization(
    ...
    quote! {
        let #field_name: #field_type = compote::FromContextValue::from_context_value(&__remaining_value, tracker)?;
    }
}
...
fn generate_from_context_deserialization(
    ...
    // Fall back to __compote_value.context() if no fields exist
    // NOTE: We reference __compote_value (saved before field deserialization)
                ...
                quote! {
                    let #field_name: #field_type = {
                        ...
                        ctx.source.file_path()
                            .map(|p| p.to_path_buf())
                            .ok_or_else(|| compote::Error::MissingField {
                                path: tracker.current_path(),
...
/// The function receives &compote::Context<S, L> and returns the field type.
    ...
    for field in fields {
        ...
        if attrs.skip_if_empty {
            let helper_name = syn::Ident::new(
                &format!("__compote_skip_if_empty_{}", field_name),
                field_name.span(),
            );
        ...
        if attrs.skip_if_empty_recursive {
            let helper_name = syn::Ident::new(
                &format!("__compote_skip_if_empty_recursive_{}", field_name),
                field_name.span(),
            );
        ...
        if attrs.skip_if_default {
            let helper_name = syn::Ident::new(
                &format!("__compote_skip_if_default_{}", field_name),
                field_name.span(),
            );
        ...
        if let Some(ref custom_fn) = attrs.skip_if {
            ...
            let helper_name = syn::Ident::new(
                &format!("__compote_skip_if_{}", field_name),
                field_name.span(),
            );
...
fn generate_coercion_code(
    ...
    let coerce_fn_ident: proc_macro2::TokenStream =
        format!("compote::coerce::{}", coerce_fn).parse().unwrap();
...
fn generate_duration_deserialization(
    ...
    if let Some(default_expr) = default_tokens {
        ...
        quote! {
            {
                let mut __parse_error: Option<compote::Error> = None;
                let mut __temp_value: #field_type = match field_value {
                    compote::ContextValue::String(s, _) => {
                        match compote::transform::parse_duration_to_nanos(s) {
                            Ok(nanos) => {
            ...
            quote! {
                {
                    ...
                    let __empty_obj: compote::ContextValue<__CompoteS, __CompoteL> = compote::ContextValue::object(
                        __CompoteIndexMap::default(),
                        compote::Context::new(
                            __CompoteS::programmatic(),
                            __CompoteL::default(),
                        )
                    );
                    ...
                    match compote::FromContextValue::from_context_value(&__empty_obj, &mut __probe_tracker) {
                        Ok(v) => v,
    ...
    let allow_single_wrapping = if let Some(ref scalar_field) = attrs.allow_single {
        ...
        if !scalar_field.is_empty() {
            Some(quote! {
                ...
                let field_value: __CompoteCow<'_, compote::ContextValue<__CompoteS, __CompoteL>> = match field_value {
                    compote::ContextValue::String(_, _) | compote::ContextValue::Int(_, _) | compote::ContextValue::Float(_, _) | compote::ContextValue::Bool(_, _) => {
                        // Scalar input - wrap as {scalar_field: value}
                        let mut wrapped_obj = __CompoteIndexMap::default();
                        wrapped_obj.insert(#scalar_field.to_string(), field_value.clone());
                        __CompoteCow::Owned(compote::ContextValue::object(wrapped_obj, field_value.context().clone()))
                    }
                    _ => __CompoteCow::Borrowed(field_value),
                };
                ...
                let #field_name = if let Some(field_value) = #field_lookup {
                    if field_value.is_null() {
                        if let Ok(env_value) = std::env::var(#var_name) {
                            let env_config_value: compote::ContextValue<__CompoteS, __CompoteL> = compote::ContextValue::string(
                                env_value,
                                compote::Context::new(
                                    __CompoteS::environment(),
                                    __CompoteL::default(),
                                )
                            );
    ...
    let deserialization = match (&attrs.transform, &default_tokens, &coercion_code) {
        ...
        (None, None, None) => quote! {
            let mut __temp_value: #field_type = {
                let result = compote::FromContextValue::from_context_value(field_value, tracker);
                result?
            };
            #transform_after_code
            #validation_code
        ...
        (None, Some(default_expr), None) => {
            ...
            quote! {
                let mut __temp_value: #field_type = match compote::FromContextValue::from_context_value(field_value, tracker) {
                    Ok(value) => value,
                    Err(e) => {
                        #error_handling
                    }
        ...
        (Some(t), None, coerce_opt) => {
            ...
            let deser_code = if coerce_opt.is_some() {
                ...
                quote! {
                    {
                        let result = compote::FromContextValue::from_context_value(&transformed_value, tracker);
                        result?
                    }
                }
            };
        ...
        (Some(t), Some(default_expr), coerce_opt) => {
            ...
            let deser_code = if coerce_opt.is_some() {
                ...
                // Standard deserialization after transform
                quote! {
                    match compote::FromContextValue::from_context_value(&transformed_value, tracker) {
                        Ok(value) => value,
                        Err(e) => {
                    ...
                    compote::ContextValue::Array(arr, _) => {
                        ...
                        for (__idx, __item) in arr.iter().enumerate() {
                            ...
                            match __item {
                                ...
                                compote::ContextValue::Object(obj_map, _) => {
                                    for (__key, __val) in obj_map {
                                        tracker.push_field(__key);
                                        match <#value_type as compote::FromContextValue<__CompoteS, __CompoteL>>::from_context_value(__val, tracker) {
                                ...
                                _ => {
                                    let __err = compote::Error::TypeMismatch {
                                        path: tracker.current_path(),
            ...
            let #field_name = if let Some(field_value) = #field_lookup {
                ...
                if field_value.is_null() {
                    ...
                    if let Ok(env_value) = std::env::var(#var_name) {
                        ...
                        #deprecation_warning
                        let mut __temp_value: #field_type = {
                            let result = compote::FromContextValue::from_context_value(&env_config_value, tracker);
                            result?
                        };
                        #transform_after_code
                        #validation_code
