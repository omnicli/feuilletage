//! Attribute type definitions and parsing for the `#[compote(...)]` namespace.
//!
...
pub(crate) struct ContainerAttributes {
    ...
    /// When true, variants use #[compote(variant = ...)] to specify match conditions
    ...
    /// By default, compote generates Deserialize for supported types (external_tag enums).
    ...
    /// Bare names resolve to `compote::transform::*`; qualified paths are used verbatim.
...
/// Parse container-level attributes like #[compote(tag = "type")] or #[compote(untagged)]
...
fn try_parse_container_attributes(attrs: &[syn::Attribute]) -> syn::Result<ContainerAttributes> {
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
                    ...
                    while !content.is_empty() {
                        ...
                        if ident == "key" {
                            ...
                            return Err(syn::Error::new(
                                ident.span(),
                                "unknown compote allow_map option",
                            ));
                        }
            ...
            Err(meta.error("unknown compote container attribute"))
        })?;
    }
...
pub(crate) fn parse_container_attributes(attrs: &[syn::Attribute]) -> ContainerAttributes {
    try_parse_container_attributes(attrs)
        .expect("compote attributes were validated before code generation")
}

/// Represents a literal value or predicate that a variant can match against.
/// Used with `#[compote(variant = ...)]` for value-matched enums.
#[derive(Clone, Debug)]
...
/// Parse variant-level attributes like #[compote(rename = "text")] or #[compote(alias = "alt")]
fn try_parse_variant_attributes(attrs: &[syn::Attribute]) -> syn::Result<VariantAttributes> {
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            ...
            Err(meta.error("unknown compote variant attribute"))
        })?;
    }
...
/// - `#[compote(duration)]` - default unit is seconds ("s")
/// - `#[compote(duration(ms))]` - shorthand: specify target unit (just the identifier)
/// - `#[compote(duration(unit = ms))]` - explicit: specify target unit with named parameter
///
...
fn try_parse_field_config_attributes(
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            ...
            // - #[compote(allow_single)] - flag form for Vec (empty string)
            // - #[compote(allow_single = "field")] - field name for struct fields
            if meta.path.is_ident("allow_single") {
            ...
            // Syntax: #[compote(duration)] or #[compote(duration(ms))] or #[compote(duration(unit = ms))]
                ...
                if meta.input.peek(syn::token::Paren) {
                    ...
                    while !content.is_empty() {
                        ...
                        if ident == "refs" {
                            ...
                            return Err(syn::Error::new(
                                ident.span(),
                                "unknown compote template option",
                            ));
                        }
            ...
            Err(meta.error("unknown compote field attribute"))
        })?;
    }
...
pub(crate) fn validate_compote_attributes(input: &syn::DeriveInput) -> syn::Result<()> {
