...
/// has `#[compote(mutable_by = ["user", "local"])]`, values from `System` level
...
/// ```
/// use compote::{Error, ContextValue, ErrorTracker, FromContextValue, Value};
///
...
/// use compote::{Context, ContextValue, CustomSource, Error, ErrorTracker, FromContextValue, Level};
///
...
///
/// This trait is automatically implemented for types that use `#[compote(allow_map(key = field))]`
...
/// # How it works
///
/// When a `Vec<T>` field has `#[compote(allow_map)]` and receives a map as input, it needs
...
// Unit tests have been moved to compote/tests/unit/de_test.rs
