...
//! ```no_run
//! # #[cfg(feature = "std")] {
//! use compote::{loader, Level, FromContextValue};
//!
...
//!
//! When deserializing, the builder can enforce `#[compote(mutable_by = [...])]`
...
/// use compote::{loader, Format, Level, Source};
///
...
/// ```no_run
/// # #[cfg(feature = "std")] {
/// use compote::{loader, Level, Source};
///
...
/// ```
/// # #[cfg(feature = "json")] {
/// use compote::context::{Context, Level, Source};
/// use compote::loader::load_json;
///
/// let ctx = Context::new(Source::Programmatic, Level::User);
/// let value = load_json(r#"{"name": "compote", "port": 8080}"#, ctx).unwrap();
...
///
/// let ctx = Context::new(Source::Programmatic, Level::User);
/// let value = load_yaml("name: compote\nport: 8080\n", ctx).unwrap();
/// assert!(value.is_object());
/// # }
/// ```
#[cfg(feature = "yaml")]
...
///
/// let ctx = Context::new(Source::Programmatic, Level::User);
/// let value = load_toml("name = \"compote\"\nport = 8080\n", ctx).unwrap();
/// assert!(value.is_object());
/// # }
/// ```
#[cfg(feature = "toml")]
...
impl<S: SourceType, L: LevelType> ConfigLoaderBuilder<S, L> {
    ...
    /// the `#[compote(mutable_by = [...])]` constraints during merge. Values from
    ...
    /// ```
    /// # #[cfg(feature = "json")] {
    /// use compote::{loader, Level, Format, FromContextValue};
    ///
    /// #[derive(Debug, compote::Config)]
    /// struct AppConfig {
    ///     #[compote(default = "DefaultApp")]
    ///     app_name: String,  // Can be set by any level
    ///
    ///     #[compote(mutable_by = ["user", "local"], default = "none")]
    ...
    /// the `#[compote(mutable_by = [...])]` constraints from the target type.
...
// Unit tests have been moved to compote/tests/unit/loader_test.rs
