...
impl<S: SourceType, L: LevelType> Config<S, L> {
    ...
    /// ```
    /// # #[cfg(feature = "json")] {
    /// use compote::{Config, ContextValue, Context, Level, Source};
    ///
    /// let mut config = Config::default();
    /// config.load_json(
    ...
    /// ```
    /// # #[cfg(feature = "json")] {
    /// use compote::{Config, Context, Source, Level};
    ///
    /// let mut config = Config::default();
    /// let ctx = Context::new(Source::Programmatic, Level::User);
    ...
    /// ```
    /// # #[cfg(feature = "json")] {
    /// use compote::{Config, ContextValue, Context, Source, Level};
    ///
    /// let mut config = Config::default();
    /// let ctx = Context::new(Source::Programmatic, Level::User);
    /// config.load_json(r#"{"a": {"b": {"c": 42}}}"#, ctx);
    ...
    /// ```
    /// # #[cfg(feature = "json")] {
    /// use compote::{Config, Context, Level, Source, Format};
    ///
    /// let mut config = Config::default();
    ...
    /// use compote::{Config, Context, Level, Source, FromContextValue};
    ///
    /// // Note: compote::Config already derives Serialize
    /// #[derive(Debug, compote::Config)]
    /// struct AppConfig {
    ///     #[compote(default = "localhost")]
    ///     host: String,
    ...
    /// Compote writers through an exclusive sidecar lock, and atomically
    ...
    /// use compote::{Config, Format, Level, Source};
    ///
    ...
    ///
    /// ```no_run
    /// use compote::edit_first_existing;
    ///
    /// let paths = [
    ...
    ///
    /// ```no_run
    /// use compote::edit_first_writeable;
    ///
    /// let candidates = [
    ...
    ///     Context::new(Source::Programmatic, Level::User),
    /// );
    /// config.register_transform("name", compote::transform::trim);
    /// config.apply_transformations().unwrap();
    ///
    /// assert_eq!(config.get("name").and_then(|value| value.as_str()), Some("compote"));
    /// # }
    /// ```
...
// Unit tests have been moved to compote/tests/unit/config_test.rs
