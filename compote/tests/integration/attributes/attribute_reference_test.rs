...
fn every_parser_attribute_has_an_executable_reference_example() {
    ...
    let parser = fs::read_to_string(crate_dir.join("../compote-macros/src/attrs.rs"))
    ...
    assert!(
        missing.is_empty(),
        "parser attributes missing from executable compote/src/attributes.rs doctests: {missing:?}"
    );
}
