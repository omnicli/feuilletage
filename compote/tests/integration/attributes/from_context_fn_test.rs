...
fn is_not_local<S: compote::CustomSource, L: compote::CustomLevel>(
    ctx: &compote::Context<S, L>,
) -> bool {
    ctx.level.name() != "local"
}

fn source_path_from_context<S: compote::CustomSource, L: compote::CustomLevel>(
    ctx: &compote::Context<S, L>,
) -> Option<PathBuf> {
    ctx.source.file_path().map(|p| p.to_path_buf())
}

fn level_name_upper<S: compote::CustomSource, L: compote::CustomLevel>(
    ctx: &compote::Context<S, L>,
) -> String {
...
fn test_from_context_fn_bool() {
    ...
    struct WithContextFnBool {
        #[compote(default)]
        name: String,
        #[compote(from_context_fn = "is_not_local")]
        is_global: bool,
    }
...
fn test_from_context_fn_option() {
    ...
    struct WithContextFnOption {
        #[compote(default)]
        name: String,
        #[compote(from_context_fn = "source_path_from_context")]
...
fn test_from_context_fn_string() {
    ...
    struct WithContextFnString {
        #[compote(default)]
        name: String,
        #[compote(from_context_fn = "level_name_upper")]
        scope: String,
    }
