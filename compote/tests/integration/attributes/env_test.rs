...
fn test_env_variable_not_found_uses_default() {
    ...
    struct EnvConfig {
        #[compote(env = "COMPOTE_TEST_NONEXISTENT_VAR_12345", default = "default_value")]
        from_env: String,
    }

    // Make sure the env var doesn't exist
    std::env::remove_var("COMPOTE_TEST_NONEXISTENT_VAR_12345");
...
fn test_env_variable_found_is_used() {
    #[derive(DeriveConfig, Debug)]
    struct EnvConfig {
        #[compote(env = "COMPOTE_TEST_VAR_FOR_ENV_TEST", default = "default_value")]
        from_env: String,
    }

    // Set the env var
    std::env::set_var("COMPOTE_TEST_VAR_FOR_ENV_TEST", "env_value");
    ...
    // Cleanup
    std::env::remove_var("COMPOTE_TEST_VAR_FOR_ENV_TEST");
}
...
fn test_config_value_precedence_over_env() {
    #[derive(DeriveConfig, Debug)]
    struct EnvConfig {
        #[compote(env = "COMPOTE_TEST_PRECEDENCE_VAR", default = "default_value")]
        value: String,
    }

    // Set the env var
    std::env::set_var("COMPOTE_TEST_PRECEDENCE_VAR", "env_value");
    ...
    // Cleanup
    std::env::remove_var("COMPOTE_TEST_PRECEDENCE_VAR");
}
...
fn test_env_variable_with_integer() {
    #[derive(DeriveConfig, Debug)]
    struct EnvIntConfig {
        #[compote(env = "COMPOTE_TEST_INT_VAR", coerce, default = "0")]
        count: i32,
    }

    // Set the env var with numeric string
    std::env::set_var("COMPOTE_TEST_INT_VAR", "42");
    ...
    // Cleanup
    std::env::remove_var("COMPOTE_TEST_INT_VAR");
}
...
fn test_env_variable_with_boolean() {
    ...
    struct EnvBoolConfig {
        #[compote(env = "COMPOTE_TEST_BOOL_VAR", coerce, default = "false")]
        enabled: bool,
    }

    // Set the env var
    std::env::set_var("COMPOTE_TEST_BOOL_VAR", "true");
    ...
    // Cleanup
    std::env::remove_var("COMPOTE_TEST_BOOL_VAR");
}
...
fn test_required_env_fails_when_not_set() {
    ...
    struct RequiredEnvConfig {
        #[compote(env = "COMPOTE_TEST_REQUIRED_ENV_VAR")]
        required_from_env: String,
    }

    // Make sure the env var doesn't exist
    std::env::remove_var("COMPOTE_TEST_REQUIRED_ENV_VAR");
...
fn test_multiple_env_attributes() {
    ...
    struct MultiEnvConfig {
        #[compote(env = "COMPOTE_TEST_HOST", default = "localhost")]
        host: String,

        #[compote(env = "COMPOTE_TEST_PORT", coerce, default = "8080")]
        port: u16,

        #[compote(env = "COMPOTE_TEST_DEBUG", coerce, default = "false")]
        debug: bool,
    }

    // Set some env vars
    std::env::set_var("COMPOTE_TEST_HOST", "example.com");
    std::env::set_var("COMPOTE_TEST_PORT", "3000");
    ...
    // Cleanup
    std::env::remove_var("COMPOTE_TEST_HOST");
    std::env::remove_var("COMPOTE_TEST_PORT");
}
