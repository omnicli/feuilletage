...
struct BasicAliasConfig {
    #[compote(aliases = ["repo"])]
    repository: String,
}

#[derive(Debug, DeriveConfig)]
struct MultiAliasConfig {
    #[compote(aliases = ["repo", "r", "source"])]
    repository: String,
}

#[derive(Debug, DeriveConfig)]
struct SingularAndPluralAliasConfig {
    #[compote(alias = "repo", aliases = ["r", "source"])]
    repository: String,
}

#[derive(Debug, DeriveConfig)]
struct AliasWithDefaultConfig {
    #[compote(aliases = ["repo"], default = "default-repo")]
    repository: String,
}

#[derive(Debug, DeriveConfig)]
struct AliasWithValidationConfig {
    #[compote(aliases = ["num", "n"], range(1, 100))]
    number: i32,
}

#[derive(Debug, DeriveConfig)]
struct VecAliasConfig {
    #[compote(aliases = ["pkgs", "packages"], allow_single)]
    package: Vec<String>,
}
