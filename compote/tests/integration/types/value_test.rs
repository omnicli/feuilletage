//! Tests for the public `compote::Value` type as a deserialization target
//! and Default-able field type.

#![cfg(feature = "json")]

use compote::{Config, Context, Level, Source, Value};
...
// Notes on `#[compote(default)]` vs `Value::default()`:
//
// `Value::default()` returns `Value::Null`. However, the derive macro's
// `#[compote(default)]` codegen for non-Option fields tries to deserialize
...
// invoke their field-level `#[compote(default = ...)]` annotations.
//
...
// `#[compote(default = "compote::Value::Null")]`, which sidesteps the
...
fn missing_value_field_uses_default() {
    ...
    #[derive(DeriveConfig)]
    struct C {
        #[compote(default = "compote::Value::Null")]
        x: Value,
    }
...
/// Documents the bare-`#[compote(default)]` interaction with the macro's
