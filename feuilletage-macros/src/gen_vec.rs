    ...
    let normalization_code = if let Some(ref allow_map_config) = attrs.allow_map {
        ...
        let map_to_array_body = if let Some(ref scalar_as_field) = allow_map_config.scalar_as_field
        {
            ...
            quote! {
                ...
                feuilletage::ContextValue::array(items, field_value.context().clone())
            }
        } else {
        ...
        let map_to_array = quote! {
            feuilletage::ContextValue::Object(map, _) => {
                ...
                if is_single_item {
                    ...
                    feuilletage::ContextValue::array(vec![field_value.clone()], field_value.context().clone())
                } else {
        ...
        quote! {
            ...
            let array_value: feuilletage::ContextValue<__FeuilletageS, __FeuilletageL> = match field_value {
                feuilletage::ContextValue::Array(_, _) => field_value.clone(),
                #map_to_array
                #single_value_arm
            };
        }
        ...
        quote! {
            #[allow(unused_mut, unused_variables)]
            let mut was_from_map = false;
            let array_value: feuilletage::ContextValue<__FeuilletageS, __FeuilletageL> = field_value.clone();
        }
    };
                ...
                let mut result: Vec<#elem_ty> = match array_value {
                    feuilletage::ContextValue::Array(arr, _) => {
                        ...
                        for (i, item) in arr.iter().enumerate() {
                            ...
                            match feuilletage::FromContextValue::from_context_value(&transformed_item, tracker) {
