...
pub(crate) fn generate_enum_impl(
    ...
    let base = if container_attrs.untagged {
        ...
        panic!("Enums must have either #[feuilletage(tag = \"...\")], #[feuilletage(untagged)], #[feuilletage(external_tag)], or #[feuilletage(value_matched)] attribute")
    };
...
fn generate_tagged_enum_impl(
    ...
    let fallback_count = variants
        ...
        .count();
    if fallback_count > 1 {
        panic!("Only one variant can have #[feuilletage(fallback)] attribute");
    }
                ...
                quote! {
                    {
                        ...
                        let inner: #field_type = feuilletage::FromContextValue::from_context_value(value, tracker)?;
    ...
    let wildcard_handlers: Vec<proc_macro2::TokenStream> = variants.iter()
        ...
        .map(|variant| {
            ...
            let type_condition = if attrs.scalar_variant {
                quote! { matches!(value, feuilletage::ContextValue::String(_, _) | feuilletage::ContextValue::Int(_, _) | feuilletage::ContextValue::Float(_, _) | feuilletage::ContextValue::Bool(_, _)) }
            } else {
            ...
            match &variant.fields {
                ...
                Fields::Unnamed(fields) => {
                    ...
                    let allow_single_arm = if variant_attrs.allow_single {
                        quote! {
                            ...
                            if !value.is_array() && !value.is_null() {
                                let wrapped = feuilletage::ContextValue::Array(
                                    vec![value.clone()],
                                    value.context().clone(),
                                );
                                let mut try_tracker = tracker.child();
                                if let Ok(inner) = <#field_type as feuilletage::FromContextValue<__FeuilletageS, __FeuilletageL>>::from_context_value(&wrapped, &mut try_tracker) {
                                    if !try_tracker.has_errors() {
                                        tracker.commit_child(try_tracker);
                    ...
                    let type_guard = if type_str == "String" {
                        quote! { matches!(value, feuilletage::ContextValue::String(_, _)) }
                    } else if type_str == "bool" {
                        quote! { matches!(value, feuilletage::ContextValue::Bool(_, _)) }
                    ...
                    {
                        quote! { matches!(value, feuilletage::ContextValue::Int(_, _)) }
                    } else if type_str == "f64" || type_str == "f32" {
                        quote! { matches!(value, feuilletage::ContextValue::Float(_, _) | feuilletage::ContextValue::Int(_, _)) }
                    } else {
                        quote! { true }
                    };
                    ...
                    quote! {
                        {
                            ...
                            if #type_guard {
                                let mut try_tracker = tracker.child();
                                if let Ok(inner) = <#field_type as feuilletage::FromContextValue<__FeuilletageS, __FeuilletageL>>::from_context_value(value, &mut try_tracker) {
                                    if !try_tracker.has_errors() {
                                        tracker.commit_child(try_tracker);
                                        return Ok(#name::#variant_name(inner));
                                    }
        ...
        Fields::Unnamed(fields) => {
            ...
            if let Some(ref lit) = attrs.variant_value {
                ...
                quote! {
                    if #condition {
                        let mut try_tracker = tracker.child();
                        if let Ok(inner) = feuilletage::FromContextValue::from_context_value(value, &mut try_tracker) {
                            if !try_tracker.has_errors() {
                                tracker.commit_child(try_tracker);
...
fn generate_external_tag_enum_impl(
    ...
    let null_count = variants
        ...
        .count();
    if null_count > 1 {
        panic!("Only one variant can have #[feuilletage(null_variant)] or #[feuilletage(variant = null)] attribute");
    }
    ...
    let wildcard_match_handlers: Vec<proc_macro2::TokenStream> = variants.iter()
        ...
        .map(|variant| {
            ...
            let type_condition = if attrs.scalar_variant {
                ...
                quote! {
                    matches!(value, feuilletage::ContextValue::String(_, _) | feuilletage::ContextValue::Int(_, _) | feuilletage::ContextValue::Float(_, _) | feuilletage::ContextValue::Bool(_, _))
                }
            } else {
    ...
    let variant_arms: Vec<proc_macro2::TokenStream> = variants.iter()
        ...
        .map(|variant| {
            ...
            quote! {
                #(#tag_patterns)|* => {
                    let inner: #field_type = feuilletage::FromContextValue::from_context_value(inner_value, tracker)?;
                    Ok(#name::#variant_name(inner))
                }
            }
        }).collect();
    ...
    let fallback_handling = if let Some(fallback) = fallback_variant {
        ...
        quote! {
            __other_key => {
                let mut inner: #fallback_type = feuilletage::FromContextValue::from_context_value(
                    inner_value,
                    tracker,
                )?;
                #from_tag_injection
...
///
/// This approach deserializes the input into `feuilletage::Value` first, then converts
/// to `ContextValue` with a default context, and finally delegates to `FromContextValue`.
/// This ensures all feuilletage features (defaults, transforms, flatten, scalar_as, etc.)
...
pub(crate) fn generate_deserialize_via_from_context_value(
    ...
    quote! {
        impl<'__feuilletage_de> serde::Deserialize<'__feuilletage_de> for #name #ty_generics #where_clause {
            fn deserialize<__D>(__deserializer: __D) -> Result<Self, __D::Error>
            where
                __D: serde::Deserializer<'__feuilletage_de>,
            {
                let __value = <feuilletage::Value as serde::Deserialize>::deserialize(__deserializer)?;
                let __ctx_value = feuilletage::ContextValue::<feuilletage::Source, feuilletage::Level>::from(__value);
                let mut __tracker = feuilletage::ErrorTracker::new();
                <Self as feuilletage::FromContextValue<feuilletage::Source, feuilletage::Level>>::from_context_value(
                    &__ctx_value,
                    &mut __tracker,
...
/// Each variant specifies which values it matches via `#[feuilletage(variant = ...)]`.
///
/// Example:
/// ```text
...
/// pub enum SelfUpdate {
///     #[feuilletage(variant = true | "true" | "yes" | 1)]
///     True,
///     #[feuilletage(variant = false | "false" | "no" | 0)]
///     False,
    ...
    let variant_match_arms: Vec<proc_macro2::TokenStream> = variants.iter()
        ...
        .map(|variant| {
            ...
            if variant_attrs.variant_matches.is_empty() && variant_attrs.variant_fn.is_none() {
                panic!(
                    "value_matched enum variant '{}' must have #[feuilletage(variant = ...)] or #[feuilletage(variant_fn = ...)] attribute",
                    variant_name
                );
            }
...
impl VariantMatch {
    ...
    fn to_match_condition(&self) -> proc_macro2::TokenStream {
        match self {
            VariantMatch::Bool(b) => {
                quote! { matches!(value, feuilletage::ContextValue::Bool(v, _) if *v == #b) }
            }
            VariantMatch::String(s) => {
                quote! { matches!(value, feuilletage::ContextValue::String(v, _) if v == #s) }
            }
            VariantMatch::Int(i) => {
                quote! { matches!(value, feuilletage::ContextValue::Int(v, _) if *v == #i) }
            }
            VariantMatch::Float(f) => {
                quote! { matches!(value, feuilletage::ContextValue::Float(v, _) if (*v - #f).abs() < f64::EPSILON) }
            }
            VariantMatch::Truthy => {
                quote! {
                    match value {
                        feuilletage::ContextValue::Bool(true, _) => true,
                        feuilletage::ContextValue::String(s, _) => matches!(s.to_lowercase().as_str(), "true" | "yes" | "y" | "on" | "1"),
                        feuilletage::ContextValue::Int(i, _) => *i != 0,
                        _ => false,
                    }
                }
            }
            VariantMatch::Falsy => {
                quote! {
                    match value {
                        feuilletage::ContextValue::Bool(false, _) => true,
                        feuilletage::ContextValue::String(s, _) => matches!(s.to_lowercase().as_str(), "false" | "no" | "n" | "off" | "0"),
                        feuilletage::ContextValue::Int(0, _) => true,
                        _ => false,
                    }
                }
            }
            VariantMatch::Null => {
                quote! { matches!(value, feuilletage::ContextValue::Null(_)) }
            }
            VariantMatch::AnyString => {
                quote! { matches!(value, feuilletage::ContextValue::String(_, _)) }
            }
            ...
            VariantMatch::AnyFloat => {
                quote! { matches!(value, feuilletage::ContextValue::Float(_, _)) }
            }
            VariantMatch::AnyBool => {
                quote! { matches!(value, feuilletage::ContextValue::Bool(_, _)) }
            }
            ...
            VariantMatch::StartsWith(prefix) => {
                quote! {
                    matches!(value, feuilletage::ContextValue::String(s, _) if s.starts_with(#prefix))
                }
            }
            VariantMatch::EndsWith(suffix) => {
                quote! {
                    matches!(value, feuilletage::ContextValue::String(s, _) if s.ends_with(#suffix))
                }
            }
            VariantMatch::Contains(substring) => {
                quote! {
                    matches!(value, feuilletage::ContextValue::String(s, _) if s.contains(#substring))
                }
            }
            VariantMatch::Range(min, max) => {
                ...
                quote! {
                    match value {
                        ...
                        feuilletage::ContextValue::Float(f, _) => (#min_f..=#max_f).contains(f),
                        _ => false,
                    }
                }
            }
            ...
            VariantMatch::Regex(pattern) => {
                quote! {
                    matches!(value, feuilletage::ContextValue::String(s, _) if {
                        // Compile regex at runtime - cached by user if needed
                        feuilletage::regex::Regex::new(#pattern).map(|re| re.is_match(s)).unwrap_or(false)
                    })
                }
            }
