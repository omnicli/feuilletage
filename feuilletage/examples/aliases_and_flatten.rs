...
//!
//! Feuilletage Solution:
//! - `#[feuilletage(aliases = ["alt_name"])]` for field aliases
//! - `#[feuilletage(flatten)]` for struct composition
//! - `#[feuilletage(rename = "key")]` for key renaming
...
struct NestedConfig {
    ...
    #[feuilletage(aliases = ["pattern"])]
    name: Option<String>,
    /// Field with a short alias ("ext" also accepted)
    #[feuilletage(aliases = ["ext"])]
...
struct AliasedFieldsConfig {
    ...
    #[feuilletage(aliases = ["dest", "out", "output"])]
