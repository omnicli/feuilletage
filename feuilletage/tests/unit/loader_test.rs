...
//!
//! Extracted from feuilletage/src/loader.rs
...
use feuilletage::{Context, ContextValue};
use feuilletage::{Error, Format, Level, Source};
...
fn test_loader_builder_uses_custom_level_priorities() {
    ...
    impl feuilletage::CustomLevel for CustomPriorityLevel {
        fn name(&self) -> &str {
            match self {
                Self::High => "high",
                Self::Low => "low",
    ...
    let config = feuilletage::ConfigLoaderBuilder::<Source, CustomPriorityLevel>::new()
        .load_str(
            r#"{"value": "high"}"#,
            Format::Json,
...
fn test_loader_builder_load_file_sets_detected_format_context() {
    ...
    fn context_format<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
        context: &Context<S, L>,
    ) -> Format {
    ...
    struct FileConfig {
        name: String,
        #[feuilletage(from_context_fn = "context_format")]
        format: Format,
    }
...
fn test_direct_load_file_with_format_accepts_unknown_extension() {
    ...
    let value: ContextValue =
        feuilletage::loader::load_file_with_format(file.path(), Format::Yaml, Level::User)
            .unwrap()
            .unwrap();
...
fn test_load_file_auto_uses_json_toml_yaml_order() {
    ...
    let json_value: ContextValue = feuilletage::loader::load_file_auto(json.path(), Level::User)
        .unwrap()
        .unwrap();
    ...
    let toml_value: ContextValue = feuilletage::loader::load_file_auto(toml.path(), Level::User)
        .unwrap()
        .unwrap();
    ...
    let yaml_value: ContextValue = feuilletage::loader::load_file_auto(yaml.path(), Level::User)
        .unwrap()
        .unwrap();
...
fn test_load_file_auto_aggregates_parser_failures_in_attempt_order() {
    ...
    let error =
        feuilletage::loader::load_file_auto::<Source, Level>(file.path(), Level::User).unwrap_err();
...
fn test_load_file_auto_with_only_json_enabled() {
    ...
    let value: ContextValue = feuilletage::loader::load_file_auto(file.path(), Level::User)
        .unwrap()
        .unwrap();
...
mod warning_path_tests {
    ...
    use feuilletage::{ErrorTracker, FromContextValue, MutabilityHashMap};
...
mod mutability_tests {
    use super::*;
    use feuilletage::de::MutabilityInfo;
    use feuilletage::error::Error;
    use feuilletage::MutabilityHashMap;
    ...
    impl feuilletage::FromContextValue for TestConfig {
        fn from_context_value(
            value: &ContextValue,
            tracker: &mut feuilletage::error::ErrorTracker,
        ) -> Result<Self, feuilletage::error::Error> {
