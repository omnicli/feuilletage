...
//!
//! Extracted from feuilletage/src/ser.rs
...
fn test_to_json_function() {
    ...
    let json = feuilletage::to_json(&config).unwrap();
    assert!(json.contains("\"name\""));
    assert!(json.contains("\"test\""));
    assert!(json.contains("\"count\""));
    assert!(json.contains("42"));
...
fn test_to_json_compact_function() {
    ...
    let json = feuilletage::to_json_compact(&config).unwrap();
    assert!(json.contains("\"name\""));
    assert!(json.contains("\"test\""));
...
fn test_to_yaml_function() {
    ...
    let yaml = feuilletage::to_yaml(&config).unwrap();
    assert!(yaml.contains("name"));
    assert!(yaml.contains("test"));
    assert!(yaml.contains("count"));
    assert!(yaml.contains("42"));
...
fn test_to_toml_function() {
    ...
    let toml = feuilletage::to_toml(&config).unwrap();
...
fn test_to_format_function() {
    ...
    let json = feuilletage::to_format(&config, Format::Json).unwrap();
    assert!(json.contains("\"name\""));
    assert!(json.contains("\"test\""));
}
...
fn test_config_serialize() {
    ...
    // feuilletage::Config already derives Serialize, so no need to add it separately
