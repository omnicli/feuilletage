...
impl feuilletage::CustomSource for TestSource {
...
impl feuilletage::CustomLevel for TestLevel {
    fn name(&self) -> &str {
        match self {
            TestLevel::Base => "base",
            TestLevel::Team => "team",
...
struct SimpleConfig {
    #[feuilletage(default = "default_name")]
    name: String,
...
struct ValidatedConfig {
    #[feuilletage(default = "user", regex = "^[a-z]+$")]
    username: String,
    #[feuilletage(default = 18, range(min = 0, max = 150))]
    age: u8,
}
...
fn cv_string_cs(
    s: impl Into<String>,
    source: TestSource,
) -> ContextValue<TestSource, feuilletage::Level> {
    ContextValue::string(
        s.into(),
        Context::new(source, feuilletage::Level::default()),
    )
}

fn cv_int_cs(i: i64, source: TestSource) -> ContextValue<TestSource, feuilletage::Level> {
    ContextValue::int(i, Context::new(source, feuilletage::Level::default()))
}

fn cv_array_cs(
    items: Vec<ContextValue<TestSource, feuilletage::Level>>,
    source: TestSource,
) -> ContextValue<TestSource, feuilletage::Level> {
    ContextValue::array(items, Context::new(source, feuilletage::Level::default()))
}

fn cv_object_cs(
    map: indexmap::IndexMap<String, ContextValue<TestSource, feuilletage::Level>>,
    source: TestSource,
) -> ContextValue<TestSource, feuilletage::Level> {
    ContextValue::object(map, Context::new(source, feuilletage::Level::default()))
}
...
fn cv_string_cl(
    s: impl Into<String>,
    level: TestLevel,
) -> ContextValue<feuilletage::Source, TestLevel> {
    ContextValue::string(
        s.into(),
        Context::new(feuilletage::Source::default(), level),
    )
}

fn cv_int_cl(i: i64, level: TestLevel) -> ContextValue<feuilletage::Source, TestLevel> {
    ContextValue::int(i, Context::new(feuilletage::Source::default(), level))
}

fn cv_array_cl(
    items: Vec<ContextValue<feuilletage::Source, TestLevel>>,
    level: TestLevel,
) -> ContextValue<feuilletage::Source, TestLevel> {
    ContextValue::array(items, Context::new(feuilletage::Source::default(), level))
}

fn cv_object_cl(
    map: indexmap::IndexMap<String, ContextValue<feuilletage::Source, TestLevel>>,
    level: TestLevel,
) -> ContextValue<feuilletage::Source, TestLevel> {
    ContextValue::object(map, Context::new(feuilletage::Source::default(), level))
}
...
mod custom_source_default_level {
    ...
    type CV = ContextValue<TestSource, feuilletage::Level>;
...
mod default_source_custom_level {
    ...
    type CV = ContextValue<feuilletage::Source, TestLevel>;
...
mod custom_source_custom_level {
    ...
    fn context_preserves_custom_source_and_level() {
        ...
        assert_eq!(
            feuilletage::CustomLevel::name(&value.context().level),
            "project"
        );
    }
...
mod mutable_by_custom_levels {
    use super::*;
    use feuilletage::{Config as DeriveConfig, ConfigLoaderBuilder, Format};
    ...
    struct MutableByTestConfig {
        ...
        #[feuilletage(default = "default_any")]
        any_level: String,

        /// Only base level can set this
        #[feuilletage(mutable_by = ["base"], default = "default_base_only")]
        base_only: String,

        /// Only team level can set this
        #[feuilletage(mutable_by = ["team"], default = "default_team_only")]
        team_only: String,

        /// Only project level can set this
        #[feuilletage(mutable_by = ["project"], default = "default_project_only")]
        project_only: String,

        /// Only user level can set this
        #[feuilletage(mutable_by = ["user"], default = "default_user_only")]
        user_only: String,

        /// Team or project can set
        #[feuilletage(mutable_by = ["team", "project"], default = "default_team_project")]
        team_or_project: String,

        /// Base, team, or user can set (skip project)
        #[feuilletage(mutable_by = ["base", "team", "user"], default = "default_skip_project")]
        skip_project: String,
    }
...
"#;
    ...
    fn mutable_by_enforces_custom_level_names() {
        ...
        let mut loader = ConfigLoaderBuilder::<feuilletage::Source, TestLevel>::new()
...
mod mixed_scenarios {
    ...
    impl FromContextValue<TestSource, TestLevel> for PackageOnlyConfig {
        fn from_context_value(
            ...
            let obj = value
                .as_object()
                .ok_or_else(|| feuilletage::Error::TypeMismatch {
