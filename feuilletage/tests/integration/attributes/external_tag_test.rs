...
enum BasicTool {
    #[feuilletage(rename = "homebrew")]
    Homebrew(HomebrewConfig),
    #[feuilletage(rename = "python")]
    Python(PythonConfig),
}
...
enum ToolWithAliases {
    #[feuilletage(rename = "homebrew", aliases = ["brew", "hb"])]
    Homebrew(HomebrewConfig),

    #[feuilletage(rename = "python", alias = "py")]
    Python(PythonConfig),
}
...
enum RecursiveTool {
    ...
    #[feuilletage(rename = "and")]
    And(CompoundConfig),

    #[feuilletage(rename = "or")]
    Or(CompoundConfig),
...
fn test_external_tag_serialize_basic() {
    ...
    let json = feuilletage::to_json_compact(&tool).unwrap();
    assert!(
...
fn test_null_variant_serialization() {
    ...
    let json = feuilletage::to_json_compact(&filter).unwrap();
...
/// Test that #[feuilletage(scalar)] works as alias for #[feuilletage(scalar_variant)]
#[derive(Debug, feuilletage::Config, PartialEq)]
#[feuilletage(external_tag)]
enum AliasTest {
    #[feuilletage(scalar)] // Using short form
    Direct(String),
...
enum StringFilterUnifiedNull {
    #[feuilletage(variant = null)] // New unified syntax
    Any,

    #[feuilletage(scalar_variant)] // Legacy syntax for scalar
    Glob(String),
...
enum StringFilterUnifiedScalar {
    #[feuilletage(null_variant)] // Legacy syntax for null
    Any,

    #[feuilletage(variant = any_scalar)] // New unified syntax
    Glob(String),
...
enum StringFilterFullyUnified {
    #[feuilletage(variant = null)] // New unified syntax for null
    Any,

    #[feuilletage(variant = any_scalar)] // New unified syntax for scalar
    Glob(String),
...
enum UnifiedOptionalValue {
    ...
    #[feuilletage(variant = any_scalar)]
    Value(String),
...
#[feuilletage(external_tag)]
enum Command {
    #[feuilletage(variant = "help" | "h" | "?")]
    Help,
    #[feuilletage(variant = "version" | "v")]
    Version,
    #[feuilletage(variant = any_string)] // fallback for other strings
    Custom(String),
...
fn test_exact_string_serialization() {
    ...
    let json = feuilletage::to_json_compact(&cmd).unwrap();
    assert_eq!(
        json, r#""help""#,
...
fn test_exact_bool_serialization() {
    ...
    let json = feuilletage::to_json_compact(&toggle).unwrap();
...
#[feuilletage(external_tag)]
enum Priority {
    #[feuilletage(variant = 1)]
    High,
    #[feuilletage(variant = 2)]
    Medium,
    #[feuilletage(variant = 3)]
    Low,
    #[feuilletage(variant = any_int)] // other integers
    Custom(i64),
...
fn test_exact_int_serialization() {
    ...
    let json = feuilletage::to_json_compact(&priority).unwrap();
...
#[feuilletage(external_tag)]
enum Feature {
    #[feuilletage(variant = truthy)]
    Enabled,
    #[feuilletage(variant = falsy)]
    Disabled,
...
fn test_truthy_serializes_as_true() {
    ...
    let json = feuilletage::to_json_compact(&feature).unwrap();
...
#[feuilletage(external_tag)]
enum Threshold {
    #[feuilletage(variant = 0.0)]
    Zero,
    #[feuilletage(variant = 1.0)]
    Full,
    #[feuilletage(variant = 0.5)]
    Half,
...
fn test_exact_float_serialization() {
    ...
    let json = feuilletage::to_json_compact(&threshold).unwrap();
...
#[feuilletage(external_tag)]
enum MatchOrder {
    #[feuilletage(variant = "special")]
    Special,
    #[feuilletage(variant = any_string)] // should not match "special"
    Other(String),
}
...
enum StringFilterWithStar {
    #[feuilletage(variant = null)]
    Any,
    #[feuilletage(variant = "*")] // exact string "*"
    All,
    #[feuilletage(variant = any_string)] // any other string
    Glob(String),
    Contains(String),
}
...
#[feuilletage(external_tag)]
enum StartsWithTest {
    #[feuilletage(variant = starts_with("run:"))]
    RunUnit,

    #[feuilletage(variant = starts_with("exec:"))]
    RunNewtype(String),
...
#[feuilletage(external_tag)]
enum EndsWithTest {
    #[feuilletage(variant = ends_with(".rs"))]
    RustFile(String),

    #[feuilletage(variant = ends_with(".py"))]
    PythonFile(String),
...
#[feuilletage(external_tag)]
enum RangeIntTest {
    #[feuilletage(variant = range(1, 10))]
    SmallInt(i64),

    #[feuilletage(variant = range(11, 100))]
    MediumInt(i64),
...
#[feuilletage(external_tag)]
enum RangeFloatTest {
    #[feuilletage(variant = range(0.0, 1.0))]
    Normalized(f64),
...
enum RangeCoerceTest {
    #[feuilletage(variant = range(1, 100))]
...
enum NegativeRangeTest {
    #[feuilletage(variant = range(-100, -1))]
    Negative(i64),

    #[feuilletage(variant = range(0, 100))]
    Positive(i64),
...
#[feuilletage(external_tag)]
enum RegexTest {
    #[feuilletage(variant = regex(r"^v\d+\.\d+\.\d+$"))]
    SemVer(String),

    #[feuilletage(variant = regex(r"^[a-z0-9_]+$"))]
    Identifier(String),
...
fn is_special_command<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
) -> bool {
    matches!(value, feuilletage::ContextValue::String(s, _) if s == "!special")
}

/// Custom predicate function - checks if value starts with !
fn is_important<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
) -> bool {
    matches!(value, feuilletage::ContextValue::String(s, _) if s.starts_with('!'))
}

#[derive(Debug, feuilletage::Config, PartialEq)]
#[feuilletage(external_tag)]
enum CustomPredicateTest {
    #[feuilletage(variant = predicate("is_special_command"))]
    Special,

    #[feuilletage(variant = predicate("is_important"))]
    Important(String),
...
fn parse_key_value<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
) -> Option<CustomData> {
...
fn parse_custom_object<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
) -> Option<CustomData> {
    if let feuilletage::ContextValue::Object(map, _) = value {
        if let Some(custom_val) = map.get("custom") {
            if let feuilletage::ContextValue::String(s, _) = custom_val {
                return Some(CustomData {
                    value: s.to_string(),
                    count: s.len() as i64,
                });
...
enum ParseExtractorTest {
    #[feuilletage(variant = parse("parse_key_value"))]
    KeyValue(CustomData),

    #[feuilletage(variant = parse("parse_custom_object"))]
...
enum MatchOrderWithPredicates {
    #[feuilletage(variant = "exact")]
    Exact,

    #[feuilletage(variant = truthy)]
    Truthy,

    #[feuilletage(variant = starts_with("prefix:"))]
    Prefix(String),
...
enum ComplexCommand {
    ...
    #[feuilletage(variant = ends_with(".sh"))]
    Script(String),
...
fn parse_key_value_pair<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
...
fn parse_triple<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
...
fn parse_quad<S: feuilletage::CustomSource, L: feuilletage::CustomLevel>(
    value: &feuilletage::ContextValue<S, L>,
