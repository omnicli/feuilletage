...
use feuilletage::{Config, Context, ContextValue, Format, Level, MutabilityConstraint, Source};
...
struct TestConfig {
    ...
    #[feuilletage(mutable_by = ["system"], default = "default_system_only")]
    system_only: String,
    ...
    #[feuilletage(mutable_by = ["local"], default = "default_local_only")]
    local_only: String,
    ...
    #[feuilletage(mutable_by = ["system", "user"], default = "default_system_user")]
    system_or_user: String,

    /// System or local can set, but NOT user
    #[feuilletage(mutable_by = ["system", "local"], default = "default_system_local")]
    system_or_local: String,

    /// User or local can set, but NOT system
    #[feuilletage(mutable_by = ["user", "local"], default = "default_user_local")]
    ...
    #[feuilletage(mutable_by = ["user"], default = "0")]
    ...
    #[feuilletage(mutable_by = ["user", "local"], default)]
    nested: NestedConfig,
}

#[derive(DeriveConfig, Debug, PartialEq, Default)]
struct NestedConfig {
    #[feuilletage(default = "default_nested_value")]
    value: String,
...
struct ContainerMutableConfig {
    #[feuilletage(default = "default_inherited")]
    inherited: String,

    #[feuilletage(rename = "renamed", default = "default_renamed")]
    inherited_renamed: String,

    #[feuilletage(mutable_by = ["system"], default = "default_override")]
    overridden: String,
}
...
struct NestedOverrideConfig {
    #[feuilletage(default = "user-default")]
    inherited: String,

    #[feuilletage(mutable_by = ["local"], default = "local-default")]
    overridden: String,
}

#[derive(DeriveConfig, Debug, PartialEq)]
struct RenamedNestedParent {
    #[feuilletage(rename = "policy", default, nested)]
    nested_config: NestedOverrideConfig,

    #[feuilletage(default = "unrestricted-default")]
    unrestricted: String,
}

#[derive(DeriveConfig, Debug, PartialEq)]
struct FlattenedNestedParent {
    #[feuilletage(flatten, nested)]
    nested_config: NestedOverrideConfig,
}

#[derive(DeriveConfig, Debug, Default, PartialEq)]
#[feuilletage(scalar_as = "unrestricted")]
struct ShapeChangingNestedConfig {
    #[feuilletage(mutable_by = ["user"], default = "protected-a-default")]
    protected_a: String,

    #[feuilletage(mutable_by = ["user"], default = "protected-b-default")]
    protected_b: String,
...
fn test_merge_with_immutability() {
    ...
    if let feuilletage::ContextValue::Object(map, _) = config.root() {
        if let Some(name) = map.get("name") {
            if let feuilletage::ContextValue::String(s, _) = name {
                assert_eq!(s, "original", "Immutable field should not change");
            }
        }
        if let Some(count) = map.get("count") {
            if let feuilletage::ContextValue::Int(i, _) = count {
