...
struct ExplicitOrderByConfig {
    #[feuilletage(allow_map(key = "name", scalar_as = "version", order_by = "name"))]
    tools: Vec<Tool>,
}
...
struct FlagFormOrderByConfig {
    ...
    #[feuilletage(allow_map(order_by = "name"))]
...
struct TransparentOrderByConfig {
    #[feuilletage(allow_single, allow_map(order_by = "name"))]
    tools: Vec<ToolWithTrait>,
}
...
struct FlagFormOrderByFnConfig {
    #[feuilletage(allow_map(order_by_fn = "sort_tools_by_name_desc"))]
    tools: Vec<ToolWithTrait>,
}
