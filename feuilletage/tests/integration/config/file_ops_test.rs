...
use feuilletage::{Config, Context, ContextValue, Format, Level, Source};
use std::fs;
#[cfg(feature = "json")]
use std::sync::mpsc;
#[cfg(feature = "json")]
...
fn test_unknown_extension_uses_config_default_but_recognized_extension_wins() {
    ...
    config.at("name").set("feuilletage").unwrap();
...
fn test_config_load_file_auto_preserves_detected_format() {
    let dir = temp_dir();
    let path = dir.path().join("config");
    fs::write(&path, "name = \"feuilletage\"\n").unwrap();
...
fn test_edit_file_creates_new_file() {
    ...
    let result = feuilletage::edit_file(&path, |config| {
...
fn test_edit_file_preserves_existing_values() {
    ...
    feuilletage::edit_file(&path, |config| {
...
fn test_concurrent_edits_are_serialized() {
    ...
    let first = thread::spawn(move || {
        feuilletage::edit_file(&first_path, |config| {
    ...
    let second = thread::spawn(move || {
        feuilletage::edit_file(&second_path, |config| {
            let count = match config.get("count") {
                Some(ContextValue::Int(count, _)) => *count,
                _ => panic!("count should be an integer"),
            };
...
fn test_explicit_file_apis_accept_unknown_extension() {
    ...
    feuilletage::edit_file_with_format(&path, Format::Yaml, |config| {
...
fn test_edit_file_rejects_unknown_extension_without_explicit_format() {
    ...
    let error = feuilletage::edit_file(&path, |_| true).unwrap_err();
...
fn test_edit_file_rejects_explicit_unknown_format() {
    ...
    let error = feuilletage::edit_file_with_format(&path, Format::Unknown, |_| true).unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(!path.exists());
}
...
fn test_edit_first_existing_finds_first_file() {
    ...
    let result = feuilletage::edit_first_existing(&paths, |config| {
...
fn test_edit_first_existing_returns_none_when_not_saved() {
    ...
    let paths = [&path1, &path2];
    let result = feuilletage::edit_first_existing(&paths, |_config| {
        false // Don't save
    });
...
fn test_edit_first_existing_empty_paths_returns_error() {
    let paths: [&str; 0] = [];
    let result = feuilletage::edit_first_existing(&paths, |_| true);
