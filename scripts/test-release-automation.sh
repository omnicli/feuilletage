...
[[ $(published_commit_from_archive "$temp_dir/clean.crate" compote 1.2.3) == "$expected" ]]
[[ $(published_commit_from_archive "$temp_dir/other.crate" compote 1.2.3) == "$other" ]]
expect_failure "came from a dirty worktree" \
  published_commit_from_archive "$temp_dir/dirty.crate" compote 1.2.3
archives_equivalent "$temp_dir/clean.crate" "$temp_dir/equivalent.crate" compote 1.2.3
expect_failure "differs from the checked-out package" \
  require_archives_equivalent "$temp_dir/clean.crate" "$temp_dir/source-diff.crate" compote 1.2.3
expect_failure "differs from the checked-out package" \
  require_archives_equivalent "$temp_dir/clean.crate" "$temp_dir/manifest-diff.crate" compote 1.2.3

valid_release=$(jq -n \
  --arg tag v1.2.3 --arg title "compote v1.2.3" \
  '{tag_name: $tag, name: $title, draft: false, prerelease: false}')
validate_release_json "$valid_release" v1.2.3 "compote v1.2.3"
expect_failure "is a draft" validate_release_json \
  "$(jq '.draft = true' <<< "$valid_release")" v1.2.3 "compote v1.2.3"
expect_failure "is a prerelease" validate_release_json \
  "$(jq '.prerelease = true' <<< "$valid_release")" v1.2.3 "compote v1.2.3"
expect_failure "has title" validate_release_json \
  "$(jq '.name = "wrong"' <<< "$valid_release")" v1.2.3 "compote v1.2.3"
expect_failure "references tag" validate_release_json \
  "$(jq '.tag_name = "v9.9.9"' <<< "$valid_release")" v1.2.3 "compote v1.2.3"
...
CRATES_IO_WAIT_ATTEMPTS=3 CRATES_IO_WAIT_SECONDS=0 wait_for_publish_dry_run compote
[[ $cargo_calls == 3 ]]
grep -Fxq 'publish --locked --dry-run -p compote' "$temp_dir/cargo-calls"
unset -f cargo
...
CRATES_IO_WAIT_ATTEMPTS=3 CRATES_IO_WAIT_SECONDS=0 wait_for_crate_version compote 1.2.3
[[ $cargo_info_calls == 3 ]]
grep -Fxq 'info compote@1.2.3 --registry crates-io' "$temp_dir/cargo-info-calls"
unset -f cargo
...
CRATES_IO_WAIT_ATTEMPTS=2 CRATES_IO_WAIT_SECONDS=0 \
  expect_failure "compote 1.2.3 did not resolve from crates.io" \
  wait_for_crate_version compote 1.2.3
...
if remote_tag_commit omnicli/compote v1.2.3 >/dev/null 2>&1; then
...
expect_failure "could not query GitHub tag v1.2.3" \
  remote_tag_commit omnicli/compote v1.2.3
unset -f gh

detect_release_pr() {
  local pull_requests=$1
  jq -r \
    --arg sha "$expected" --arg repository omnicli/compote --arg app_login 'compote-release[bot]' '
    any(.[];
      .base.ref == "main"
...
release_pr_fixture=$(jq -n \
  --arg sha "$expected" \
  '[{base: {ref: "main"}, head: {ref: "release-plz-2026-08-26", repo: {full_name: "omnicli/compote"}}, user: {login: "compote-release[bot]"}, merged_at: "2026-08-26T12:00:00Z", merge_commit_sha: $sha}]')
[[ $(detect_release_pr "$release_pr_fixture") == true ]]
[[ $(detect_release_pr "$(jq '.[0].head.repo.full_name = "attacker/compote"' <<< "$release_pr_fixture")") == false ]]
...
grep -Fq 'for package in compote-macros compote; do' "$readiness"
...
# shellcheck disable=SC2016
grep -Fq 'if [[ ${needs_publish[compote-macros]} == true ]]; then' "$repo_root/scripts/publish-release.sh"
# shellcheck disable=SC2016
grep -Fq 'if [[ ${needs_publish[compote]} == true ]]; then' "$repo_root/scripts/publish-release.sh"
