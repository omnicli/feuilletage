...
crate_exists() {
  ...
  if ! status=$(curl --silent --show-error \
    --user-agent "omnicli/compote release workflow" \
...
for package in compote-macros compote; do
  if [[ "$package" == "compote-macros" ]]; then
    version=$macro_version
  else
    version=$compote_version
  fi
...
if [[ ${needs_publish[compote-macros]} == true ]]; then
  cargo publish --locked -p compote-macros
fi

if [[ ${needs_publish[compote]} == true ]]; then
  wait_for_publish_dry_run compote
