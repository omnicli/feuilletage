...
Compote uses [release-plz](https://release-plz.dev/) only to prepare version,
...
6. Missing packages are published from the clean merged commit with
   `cargo publish --locked`, never `--allow-dirty`. `compote-macros` is
   published first when needed. Before publishing `compote`, bounded retries of
   `cargo publish --locked --dry-run -p compote` prove that the exact macro
   dependency resolves from the registry.
7. The workflow creates any missing annotated package tags and reconciles
   GitHub Releases. `compote-macros` uses `macros-vX.Y.Z`; `compote` uses
...
The two workspace packages have independent SemVer versions. If only Compote
changes, the macro version remains unchanged. If the macro changes, release-plz
updates Compote's exact macro dependency, such as `=0.2.0`, and the lockfile;
that dependency change also selects Compote for release.

The macro crate's ordinary release boundary is `compote-macros/Cargo.toml`,
`compote-macros/README.md`, and its packaged files under `compote-macros/src/`.
...
Compote's exact macro requirement to match the local macro package, and runs
...
the reserved `compote` crate. From one clean, reviewed `main` commit containing
...
cargo publish --locked -p compote-macros
```

Wait until `cargo info compote-macros@0.1.0 --registry crates-io` succeeds,
then publish Compote:

```bash
cargo publish --locked -p compote
```
...
test -z "$(git status --short)"
git tag -a macros-v0.1.0 "$BOOTSTRAP_COMMIT" -m "compote-macros v0.1.0"
git tag -a v0.1.0 "$BOOTSTRAP_COMMIT" -m "compote v0.1.0"
...
installed on `omnicli/compote` with these repository permissions:
...
login (for example, `compote-release[bot]`) as the repository variable
...
```bash
cargo package --locked --list -p compote
cargo package --locked --list -p compote-macros
```
